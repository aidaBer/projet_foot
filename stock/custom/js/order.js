...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> La Date de vente est obligatoire</p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> Le Nom est obligatoire</p>');
			...
			if(clientContact == "") {
				$("#clientContact").after('<p class="text-danger"> Le champ contact est obligatoire</p>');
			...
			if(paid == "") {
				$("#paid").after('<p class="text-danger"> Le paiement est obligatoire </p>');
			...
			if(paymentStatus == "") {
				$("#paymentStatus").after('<p class="text-danger"> Le Statut de Paiement  est obligatoire </p>');
		    ...
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger"> Le nom du produit est obligatoire!! </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> La quantite est obligatoire!! </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="orders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Ajouter Nouvelle Vente </a>'+
	...
	if(orderId) {		
		...
		$.ajax({
			...
			success:function(response) {
				...
				var mywindow = window.open('', 'Sow Stock', 'height=400,width=600');
		...
		$.ajax({
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount == "") {
						$("#payAmount").after('<p class="text-danger">Champs Obligatoire</p>');
					...
					if(paymentType == "") {
						$("#paymentType").after('<p class="text-danger">Champs Obligatoire</p>');
						$("#paymentType").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentType").closest('.form-group').addClass('has-success');
					}
					...
					if(paymentStatus == "") {
						$("#paymentStatus").after('<p class="text-danger">Champs Obligatoire</p>');
						$("#paymentStatus").closest('.form-group').addClass('has-error');
					} else {
						$("#paymentStatus").closest('.form-group').addClass('has-success');
					}
